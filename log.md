...
## [2026-07-31] autoresearch build | Evaluated Agent Hub's relevance to this vault
Phase-1 build for the AutoResearch task "Watch [[agent-hub]] — evaluate whether it's relevant to this vault's multi-agent direction." Enriched the existing [[agent-hub]] entity (no new page).
- **Web-grounded the architecture (2026-07-31):** one Go binary + SQLite + one bare git repo; agents share code as **git bundles** (server validates → unbundles), query the commit **DAG** (children/leaves/lineage/diffs) instead of branch/PR/merge; a threaded **message board** for async coordination; API keys + 50 MB bundle cap + ~100 pushes/hr. Status per README: **"Work in progress. Just a sketch."**
- **Relevance verdict: conceptually load-bearing, operationally not-yet — watch, don't adopt.** Agent Hub drops exactly the primitives this vault's [[vault-autoresearch]] ratchet depends on (main, merge, reviewable PR). It's the reference *substrate* for the still-open "parallelize" rung of [[extending-the-llm-wiki]] **if** paired with a metric-gated collapse back onto `main`; near-term multi-agent gains come from [[claude-code-subagents|sub-agents]] under the existing ratchet instead. The transferable idea usable now — the async message board — already maps onto `log.md` + `results.tsv` + `nightly-queue.md`.
- Added reciprocal links: [[extending-the-llm-wiki]] (parallelize rung → agent-hub substrate) and [[vault-autoresearch]] (ratchet vs. branch-less DAG contrast).
...
## [2026-07-31] autoresearch (MODE B) | New concept page: [[test-driven-development]]
Phase-4 generative proposal (rides the morning PR). The [[ai-native-engineering-org]] cluster kept linking [[test-driven-development]] with no page owning it (≥2 inbound → passes AGENTS.md's new-page test; not thin filler). Created the page with a specific, non-generic angle: in AI-native/agentic engineering the failing test stops being a "tax" and becomes the **fake-proof executable spec the agent optimizes against** — the human-owned steering wheel — mirroring how this vault's [[vault-autoresearch]] keeps a change only if `score.py` HEALTH_DEBT drops (tests = the reward function/ratchet). Distinguished from [[adversarial-code-review]] (unanticipated defects) and [[eval-driven-model-selection]] (model-level vs change-level), with honest caveats (green ≠ correct; over-fitting to tests). Indexed under AI-native work; reciprocity already held from the two linking pages.
