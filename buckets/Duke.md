---
type: bucket
created: 2026-07-24
---
# Duke  (bucket / MOC)

Duke University — Trinity College, **class of 2027**. B.S. **Neuroscience + Statistics**,
minors **Chemistry** & **I&E**, GPA **3.91**. Coursework, research, Greek life, clubs, philanthropy.

## Organizations & activities (name the key people per group → I'll tag them)
- **Pi Kappa Alpha (PIKE)** — fraternity; Cole = *Recruitment Chair*. **Alumni network: [[Pike Alumni Directory]]** (~1,000 past brothers, curated for networking — incl. **Brad Lightcap, COO of OpenAI**). Current brothers: _(name them)_
- **Phi Delta Epsilon** — pre-med fraternity. Members: _(name them)_
- **NeuroTech Consulting Club** — Cole = *Lead Consultant* (Duke labs + Durham neurotech startups). Members: _(…)_
- **Duke Shave & Buzz** — Cole = *Sponsorship Chair*; raised **$380K+** for adolescent cancer research. People: _(…)_
- **Mission Brain** · **South Indian Club**. People: _(…)_
- **Research — CIPHER / Devinney Lab** (postoperative delirium, intraoperative EEG). Colleagues/PI: _(…)_

## People (crm/)
Known Duke contacts so far: [[Jack Carey]], [[Matt Herman]], [[James Merritt]].
_(Most of Cole's Duke friends are already in the CRM — just not yet tagged Duke. See the method below.)_

## How to add Duke people (the method)
Only ~3 of the 379 CRM contacts are auto-identifiable as Duke (via Contacts org tags), and email
is noisy (the `@duke.edu` addresses were mostly Uship *customers*, not friends). So the accurate,
fast way is **human-curated by group**: Cole names the key people under each org/activity above, and
the agent then tags each person's CRM record (`category: Duke` + `Bucket: [[Duke]]`) and links them
here. As Duke topics earn their own pages (clubs, labs, the frat), link them under Knowledge.

## Knowledge (wiki)
- _(none yet — add as Duke topics get pages.)_
